...
export default function BookingPage() {
  ...
  const { services: rawServices, loading: servicesLoading } =
    useServiceMenu();
  ...
  const getTranslatedService = (service: any) => {
    ...
    return {
      name: translatedName.includes("service_translations.")
        ? service.name
        : translatedName,
      description: translatedDesc.includes(
  ...
  const checkAvailability = async () => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-84f9c112/appointments/availability`,
        {
          method: "POST",
          headers: {
  ...
  const toggleService = (
    ...
    const isAddon =
      service?.serviceType?.toLowerCase() === "addon";

    if (isAddon && parentServiceId) {
      if (!selectedServices.includes(parentServiceId)) {
        toast.error(
          "Please select the main service first before adding this option",
        );
        return;
      }
    }
      ...
      if (!isAddon) {
        ...
        setSelectedServices(
          selectedServices.filter(
            (id) =>
              id !== serviceId && !addonsToRemove.includes(id),
          ),
        );
      } else {
        setSelectedServices(
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
      console.log(
        "📝 [BOOKING] Creating/updating customer record...",
      );
      try {
        ...
        if (customerData.success) {
          ...
          if (customerData.data?.has_valid_membership) {
            console.log(
              `💎 [BOOKING] Customer has active ${customerData.data.membership.tier} membership`,
            );
          }
        } else {
          console.warn(
            "⚠️ [BOOKING] Customer update failed:",
            customerData.error,
          );
        ...
        console.error(
          "❌ [BOOKING] Customer integration error:",
          customerError,
        );
      ...
      if (data.success) {
        ...
        const serviceNamesString = selectedServices
          .map(
            (id) =>
              services.find((s) => s.id === id)?.name ||
              "Unknown Service",
        ...
        {bookingSuccessData ? (
          ...
          <>
            <div className="text-center mb-12">
              <h1 className="text-5xl font-bold mb-4 font-serif text-[#FF9800] text-[32px]">
                {t("nav.booking")}
              </h1>
            ...
            <div className="flex items-center justify-center gap-4 mb-[24px]">
              {[1, 2, 3, 4].map((s) => (
                ...
                >
                  <div
                    className={`flex h-12 w-12 items-center justify-center rounded-full border-2 transition-all duration-300 font-bold text-lg ${
                      step >= s
                  ...
                  {s < 4 && (
                    <div
                      className={`h-0.5 w-8 md:w-20 transition-all duration-500 ${
                        step > s
                          ? "bg-[#FF9800] shadow-[0_0_10px_rgba(255,152,0,0.5)]"
                          : "bg-zinc-800"
                      }`}
                    />
                  )}
            ...
            {step === 1 && (
              <Card className="bg-zinc-900/50 backdrop-blur-md border-zinc-800 text-white shadow-2xl">
                <CardHeader>
                  <CardTitle className="text-2xl font-serif text-[#FF9800]">
                    Step 1: {t("booking_page.steps.technician")}
                  </CardTitle>
                  <CardDescription className="text-gray-400">
                    {t("booking_page.technician.desc")}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    >
                      <h3
                        className={`font-serif font-bold text-lg mb-2 ${!selectedStaff ? "text-[#FF9800]" : "text-white"}`}
                      >
                        {t(
                          "booking_page.technician.no_preference",
                        )}
                      </h3>
                      <p className="text-sm text-gray-400">
                        {t(
                          "booking_page.technician.no_preference_desc",
                        )}
                      </p>
                    </div>
                          ...
                          <div className="flex-shrink-0 relative">
                            {member.avatar ? (
                              ...
                              <div className="w-14 h-14 rounded-full bg-black border-2 border-[#FF9800]/30 flex items-center justify-center shadow-[0_0_10px_rgba(255,152,0,0.2)]">
                                <div className="w-8 h-8 rounded-full bg-[#FF9800] flex items-center justify-center text-black font-bold text-lg font-serif">
                                  ₿
                                </div>
                              </div>
                            )}
            ...
            {step === 2 && (
              <Card className="bg-zinc-900/50 backdrop-blur-md border-zinc-800 text-white shadow-2xl">
                <CardHeader>
                  <CardTitle className="text-2xl font-serif text-[#FF9800]">
                    Step 2: {t("booking_page.steps.services")}
                  </CardTitle>
                  <CardDescription className="text-gray-400">
                    {t("booking_page.steps.services_desc")}
                  </CardDescription>
                  {selectedStaff && (
                ...
                <CardContent className="space-y-8">
                  {services.length === 0 ? (
                    <p className="text-center text-gray-500 py-8">
                      {t("booking_page.services.loading")}
                    </p>
                  ) : (
                    <div className="space-y-6">
                      ...
                      <div className="relative">
                        ...
                        <Input
                          placeholder={
                            t(
                              "booking_page.services.search_placeholder",
                            ).includes("booking_page.")
                              ? "Search services..."
                              : t(
                                  "booking_page.services.search_placeholder",
                                )
                      ...
                      <div className="space-y-6 pb-40">
                        {(() => {
                          let mainServices = services.filter(
                            (s) =>
                          ...
                          if (selectedStaff) {
                            ...
                            if (
                              currentStaff &&
                              currentStaff.specialties &&
                              currentStaff.specialties.length >
                                0
                            ) {
                              const staffSpecialties =
                                currentStaff.specialties.map(
                                  (s: string) =>
                                    s
                                      .replace("+", "")
                                      .trim()
                              ...
                              mainServices =
                                mainServices.filter((s) => {
                                  const cat =
                                    s.category?.toLowerCase() ||
                                    "";
                                  const name =
                                    s.name.toLowerCase();
                                  return staffSpecialties.some(
                                    (spec: string) =>
                                      cat.includes(spec) ||
                                      spec.includes(cat) ||
                                      name.includes(spec),
                          ...
                          const filtered = mainServices.filter(
                            (s) => {
                              const translatedService =
                                getTranslatedService(s);
                              const matchesSearch =
                                searchQuery === "" ||
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              {filtered.map((service) => {
                                ...
                                const addons = services.filter(
                                  (addon) =>
                                    addon.serviceType?.toLowerCase() ===
                                      "addon" &&
                                    service.compatibleServiceIds?.includes(
                                      addon.id,
                                    ),
                                );
                                  ...
                                  >
                                    ...
                                    >
                                      <div className="flex items-start justify-between mb-3 relative z-10">
                                        <h3
                                          className={`font-serif font-bold text-lg min-h-[3.5rem] flex items-start pr-2 ${isServiceSelected ? "text-[#FF9800]" : "text-white"}`}
                                        >
                                          {
                                            translatedService.name
                                          }
                                        </h3>
                                        <span className="text-xl font-bold text-[#FF9800] flex-shrink-0">
                                          ${service.price}
                                        </span>
                                      </div>
                                      <p className="text-sm text-gray-400 mb-3 relative z-10 line-clamp-2 flex-1">
                                        {
                                          translatedService.description
                                        }
                                      </p>
                                      ...
                                      {isServiceSelected && (
                                        <div className="absolute top-0 right-0 p-2">
                                          <div className="bg-[#FF9800] rounded-full p-1">
                                            <Check className="w-3 h-3 text-black" />
                                          </div>
                                        </div>
                                      )}
                                          ...
                                          return (
                                            <div
                                              ...
                                              className={`p-4 rounded-lg border transition-all group relative ${
                                                ...
                                                isAddonSelected
                                                  ? "border-[#FF9800]/70 bg-[#FF9800]/10"
                                                  : "border-zinc-800 bg-black/30"
                                              }`}
                                            >
                                              <div className="flex items-center gap-3 mb-1.5">
                                                <span
                                                  className={`text-sm font-bold ${isAddonDisabled ? "text-gray-700" : "text-[#FF9800]"}`}
                                                >
                                                  ↳
                                                </span>
                                                <h4
                                                  className={`font-medium text-sm italic flex-1 ${
                                                    isAddonDisabled
                                                      ? "text-gray-600"
                                                      : isAddonSelected
                                                        ? "text-[#FF9800]"
                                                ...
                                                <span
                                                  className={`text-sm font-bold ${isAddonDisabled ? "text-gray-600" : "text-[#FF9800]"}`}
                                                >
                                                  +$
                                                  {addon.price}
                                                </span>
                                              ...
                                              {addon.description &&
                                                translatedAddon.description && (
                                                  <p
                                                    className={`text-xs ml-6 ${isAddonDisabled ? "text-gray-700" : "text-gray-500"}`}
                                                  >
                                                    {
                                                      translatedAddon.description
                                                    }
                                              ...
                                              {isAddonSelected &&
                                                !isAddonDisabled && (
                                                  <div className="absolute top-2 right-2">
                                                    <div className="bg-[#FF9800] rounded-full p-0.5">
                                              ...
                                              {isAddonDisabled && (
                                                <div className="absolute inset-0 flex items-center justify-center bg-black/40 rounded-lg">
                                                  <span className="text-[10px] text-gray-500 font-medium bg-black/80 px-2 py-1 rounded border border-gray-800">
                                                    Select main
                                                    service
                                                    first
                                                  </span>
                      ...
                      {services.filter((s) => {
                        ...
                        <div className="text-center py-12 text-gray-500">
                          <p>
                            {t(
                              "booking_page.services.no_category",
                            )}
                          </p>
                        </div>
                      )}
                ...
                <div className="max-w-4xl mx-auto space-y-4">
                  ...
                  {selectedServices.length > 0 && (
                    <div className="flex items-center justify-between px-2">
                      ...
                      <span className="font-serif text-3xl font-bold text-[#FF9800]">
                        ${calculateTotal()}
                      </span>
                    </div>
                  )}
                  ...
                  <div className="flex gap-4">
                    ...
                    >
                      {selectedServices.length > 0
                        ? t("booking_page.form.continue")
                        : t(
                            "booking_page.services.select_continue",
                          )}
                    </Button>
                  </div>
                </div>
            ...
            {step === 3 && (
              <Card className="bg-zinc-900/50 backdrop-blur-md border-zinc-800 text-white shadow-2xl">
                <CardHeader>
                  <CardTitle className="text-2xl font-serif text-[#FF9800]">
                    Step 3: {t("booking_page.steps.time")}
                  </CardTitle>
                  <CardDescription className="text-gray-400">
                    {t("booking_page.steps.time_desc")}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-8">
                  <div className="grid md:grid-cols-2 gap-8">
                    <div>
                      <Label className="text-gray-300 mb-2 block">
                        {t("booking_page.time.select_date")}
                      </Label>
                      <div className="p-4 bg-black/40 rounded-xl border border-zinc-800">
                        <CalendarComponent
                          ...
                          classNames={{
                            head_cell:
                              "text-gray-500 w-9 font-normal text-[0.8rem]",
                            ...
                            day_selected:
                              "bg-[#FF9800] text-black hover:bg-[#FF9800] hover:text-black focus:bg-[#FF9800] focus:bg-[#FF9800] focus:text-black font-bold",
                    ...
                    <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                      <Label className="text-gray-300 mb-2 block">
                        {t("booking_page.time.select_time")}
                      </Label>
                      {/* Dynamic Time Slots */}
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-2">
                        {checkingAvailability ? (
                          ...
                          <div className="col-span-full text-center py-4 text-gray-500 italic">
                            {t("booking_page.time.no_slots")}{" "}
                            {t(
                              "booking_page.time.choose_another_day",
                            )}
                          </div>
                        )}
                      </div>
            ...
            {step === 4 && (
              <Card className="bg-zinc-900/50 backdrop-blur-md border-zinc-800 text-white shadow-2xl">
                <CardHeader>
                  <CardTitle className="text-2xl font-serif text-[#FF9800]">
                    Step 4: {t("booking_page.steps.confirm")}
                  </CardTitle>
                  <CardDescription className="text-gray-400">
                    {t("booking_page.steps.details_desc")}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-8">
                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-6">
                      ...
                      <div className="space-y-2">
                        <Label
                          ...
                          className="text-gray-300"
                        >
                          {t("booking_page.form.notes")}
                        </Label>
                        <Textarea
                          id="notes"
                          placeholder={t(
                            "booking_page.form.notes_placeholder",
                          )}
                          value={notes}
                          onChange={(e) =>
                    ...
                    <div className="bg-black/20 p-6 rounded-xl border border-zinc-800 h-fit">
                      <h3 className="font-serif font-bold text-xl text-white mb-4 border-b border-zinc-800 pb-4">
                        {t(
                          "booking_page.summary.title",
                        ).includes("booking_page.")
                          ? "Booking Summary"
                          : t("booking_page.summary.title")}
                      </h3>
                      ...
                      <div className="space-y-4 text-sm">
                        <div className="flex justify-between items-start">
                          <span className="text-gray-400">
                            {t(
                              "services_page.service_menu.headers.service",
                            )}
                          </span>
                        ...
                        <div className="flex justify-between">
                          <span className="text-gray-400">
                            {t(
                              "booking_page.summary.date",
                            ).includes("booking_page.")
                              ? "Date"
                              : t("booking_page.summary.date")}
                          </span>
                        ...
                        <div className="flex justify-between">
                          <span className="text-gray-400">
                            {t(
                              "booking_page.summary.time",
                            ).includes("booking_page.")
                              ? "Time"
                              : t("booking_page.summary.time")}
                          </span>
                          <span className="font-medium text-white">
                            {selectedTime || "-"}
                          </span>
                        ...
                        {selectedStaff && (
                          <div className="flex justify-between">
                            <span className="text-gray-400">
                              {t(
                                "booking_page.summary.technician",
                              ).includes("booking_page.")
                                ? "Technician"
                                : t(
                                    "booking_page.summary.technician",
                        ...
                        <div className="border-t border-zinc-800 pt-4 mt-4 flex justify-between items-center">
                          <span className="text-gray-300 font-bold">
                            {t(
                              "booking_page.summary.total",
                            ).includes("booking_page.")
                              ? "Total"
                              : t("booking_page.summary.total")}
                          </span>
              ...
              <div className="fixed bottom-0 left-0 right-0 bg-zinc-900/95 backdrop-blur-md z-50 px-6 py-4 border-t border-zinc-800 shadow-[0_-4px_20px_rgba(0,0,0,0.3)]">
                <div className="max-w-4xl mx-auto flex gap-4">
                  ...
                  <Button
                    ...
                    disabled={
                      loading || !customerName || !customerPhone
                    }
                  >
                    {loading ? (
