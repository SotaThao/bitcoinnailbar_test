...
export function Chatbot() {
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        ...
        console.error(
          "Failed to fetch services for chatbot:",
          error,
        );
      }
    };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-84f9c112/chat`,
        {
          method: "POST",
          headers: {
      ...
      if (data.success) {
        ...
        if (data.ticketData) {
          console.log(
            "🎫 [CHATBOT] Received ticketData from backend:",
            {
              hasQrCodeUrl: !!data.ticketData.qrCodeUrl,
              qrCodeUrl:
                data.ticketData.qrCodeUrl?.substring(0, 50) +
                "...",
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "I'm having a bit of trouble connecting to the network right now. Please try again in a moment!",
        },
      ]);
    } finally {
      setIsLoading(false);
      ...
      <AnimatePresence>
        {isOpen && !isPaymentModalOpen && (
          <motion.div
            ...
            style={{
              boxShadow:
                "0 10px 40px -10px rgba(255, 152, 0, 0.3)",
            }}
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#151923]">
              {messages.map((msg, index) => {
                ...
                if (msg.isTicket) {
                  console.log(
                    `🎫 [CHATBOT RENDER] Message ${index} is ticket:`,
                    {
                      ...
                      <div className="w-full max-w-[90%] bg-[#0B0F19] rounded-xl border border-[#FF9800]/30 overflow-hidden">
                        ...
                        <div className="p-4 space-y-3">
                          ...
                          <div className="text-center">
                            ...
                            <h4 className="text-[#FF9800] font-bold text-sm">
                              Booking Confirmed!
                            </h4>
                          </div>
                          ...
                          <div className="space-y-2 text-xs">
                            ...
                            <div className="flex justify-between">
                              ...
                              <span className="text-white">
                                {new Date(
                                  msg.ticketData.appointmentTime,
                              ...
                              <ReactMarkdown
                                components={{
                                  ...
                                  strong: ({
                                    ...
                                    <span
                                      className="font-bold text-[#FF9800]"
                                      {...props}
                                    />
                                  ),
                                }}
                      ...
                      >
                        <ReactMarkdown
                          components={{
                            ...
                            strong: ({ node, ...props }) => (
                              <span
                                className={`font-bold ${msg.role === "assistant" ? "text-[#FF9800]" : ""}`}
                                {...props}
                              />
                            ),
                            p: ({ node, ...props }) => (
              ...
              {isLoading && <RotatingMarketingMessage />}
