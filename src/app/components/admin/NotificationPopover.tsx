...
export function NotificationPopover() {
    ...
    try {
      ...
      if (response.success && Array.isArray(response.data)) {
        ...
        const sorted = response.data
          .filter((item: any) => item && (item.appointmentTime || item.date))
          .sort((a: any, b: any) => {
        ...
        if (sorted.length > 0) {
          const lastSeenTime = localStorage.getItem('bitcoin-nail-admin-last-seen-time');
          
          let count = 0;
          if (!lastSeenTime) {
             // If never seen, count only pending appointments
             count = sorted.filter((item: any) => item.status === 'pending').length;
          } else {
             const lastSeenDate = new Date(lastSeenTime);
             count = sorted.filter((item: any) => {
               const itemDate = new Date(item.appointmentTime || item.date);
               const isNew = itemDate.getTime() > lastSeenDate.getTime();
  ...
  useEffect(() => {
    if (open && activities.length > 0) {
      ...
      const latestTime = latestItem.appointmentTime || latestItem.date;
      localStorage.setItem('bitcoin-nail-admin-last-seen-time', latestTime);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('booking-update', handleUpdate);
  }, [open]);
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="text-gray-400 hover:text-gray-900 relative">
              ...
              {activities.map((item) => (
                ...
                >
                  <div className={`mt-1 h-2 w-2 rounded-full shrink-0 ${
                    ...
                    item.status === 'pending' ? 'bg-orange-500' : 'bg-gray-300'
                  }`} />
                  <div className="flex-1 space-y-1">
                    <p className="text-sm font-medium leading-none">
                      {item.customerName || item.customer_name || 'New Customer'}
                    </p>
                    <p className="text-xs text-gray-500 line-clamp-1">
                      {item.serviceNames?.join(', ') || 'Service booking'}
                    </p>
                    <div className="flex items-center gap-2 text-[10px] text-gray-400">
                      <Clock className="h-3 w-3" />
                      <span>
                        {formatDateDisplay(item.appointmentTime || item.date)}
                      </span>
