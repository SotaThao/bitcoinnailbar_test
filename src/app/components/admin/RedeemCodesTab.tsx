...
export default function RedeemCodesTab() {
  ...
  const fetchCodes = async (silent: boolean = false) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.error(
          "❌ [ADMIN] Failed to fetch redeem codes:",
          result.error,
        );
        console.error(
          "❌ [ADMIN] Error details:",
      ...
      console.error(
        "❌ [ADMIN] Error fetching redeem codes:",
        error,
      );
  ...
  const filteredCodes = codes.filter((code) => {
    ...
    const matchesStatus =
      statusFilter === "all" || code.status === statusFilter; // ← Simplified: no need for 'completed' mapping
  ...
  const getTierBadge = (tier: string) => {
    ...
    return (
      colors[tier.toLowerCase() as keyof typeof colors] ||
