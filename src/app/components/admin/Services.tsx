...
export default function AdminServices() {
  ...
  const [mainTab, setMainTab] = useState<"services" | "menu">(
    "services",
  );
  ...
  const [isServiceSheetOpen, setIsServiceSheetOpen] =
    useState(false);
  ...
  const isCategoryDisabled =
    (activeCategory as any)?.status === "disabled";
  ...
  const handleOpenServiceSheet = (service?: Service) => {
    ...
    if (!service && isCategoryDisabled) {
      if (
        !confirm(
          "This category is currently disabled. Services in disabled categories cannot be activated. Do you still want to add a service?",
        )
      ) {
        return;
      }
  ...
  const handleToggleStatus = async (service: Service) => {
    ...
    console.log(
      `🔄 [Services] Toggling status for \"${service.name}\" from ${service.status} to ${newStatus}`,
    );
    ...
    if (result.success) {
      console.log(
        `✅ [Services] Status toggled successfully to ${newStatus}`,
      );
    } else {
  ...
  const handleQuickCreateService = async (data: {
    name: string;
    price: string;
    memberPrice: string;
  }): Promise<string | null> => {
    // Find the dedicated Add-on category if it exists
    const addonCategory = allCategories.find((cat) =>
    ...
    if (addonCategory) {
      ...
      console.log(
        "✨ Auto-creating missing Add-ons category...",
      );
  ...
  const handleSaveCategory = async (
    categoryData: Omit<ServiceCategory, "id"> | ServiceCategory,
  ): Promise<boolean> => {
    if ("id" in categoryData && categoryData.id) {
      // Update existing
      return await updateCategory(
        categoryData as ServiceCategory,
      );
    } else {
      // Add new
      return await addCategory(
        categoryData as Omit<ServiceCategory, "id">,
      );
    }
  };
  ...
  const handleDeleteCategory = async (categoryId: number) => {
    console.log(
      `🗑️ [Services.tsx] handleDeleteCategory called with ID: ${categoryId}`,
    );
    ...
    if (success) {
      ...
      console.error(
        `❌ [Services.tsx] Failed to delete category ${categoryId}`,
      );
    }
  };
  ...
  const handleReorderCategories = async (
    ...
    try {
      ...
      if (data.success) {
        console.log(
          `✅ [Services.tsx] Categories reordered successfully`,
        );
        // Refresh categories to get updated order
        await refreshCategories();
      } else {
        console.error(
          `❌ [Services.tsx] Failed to reorder categories:`,
          data.error,
        );
      }
    } catch (error) {
      console.error(
        `❌ [Services.tsx] Error reordering categories:`,
        error,
      );
    }
  };
    ...
    <AdminLayout>
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <PillTabs
          value={mainTab}
          onValueChange={(value) =>
            setMainTab(value as "services" | "menu")
          }
        >
          <PillTabsList>
              ...
              <div className="lg:col-span-3 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
                  ...
                  <Button
                    ...
                    title={
                      isCategoryDisabled
                        ? "Cannot add services to disabled category"
                        : "Add new service"
                    }
                  >
                ...
                <ServicesTable
                  ...
                  allAddons={services.filter(
                    (s) => s.serviceType === "addon",
                  )}
                  loading={loading}
