...
const getSupabaseClient = () => {
  if (!supabaseInstance) {
    console.log('📡 [GlobalListener] Creating new Supabase client instance');
...
export function GlobalRealtimeListener() {
  ...
  useEffect(() => {
    console.log('📡 [GlobalListener] Initializing Supabase Realtime...');
    ...
    const channel = supabase
      ...
      .on('broadcast', { event: 'appointment_created' }, (payload) => {
        console.log('⚡ [GlobalListener] Received appointment_created:', payload);
        handleNewAppointment(payload.payload);
      })
      .subscribe((status) => {
        console.log('📡 [GlobalListener] Subscription status:', status);
        
        if (status === 'SUBSCRIBED') {
          console.log('✅ [GlobalListener] Successfully subscribed to appointments channel');
        } else if (status === 'CHANNEL_ERROR') {
          console.error('❌ [GlobalListener] Channel subscription error');
        }
      });
    ...
    const handleNewAppointment = (appointment: any) => {
      console.log('🎉 [GlobalListener] New appointment notification:', appointment);
      
      const customerName = appointment.customerName || 'New Customer';
    ...
    const playNotificationSound = () => {
      try {
        ...
        audio.volume = 0.5;
        audio.play().catch(() => {
          console.log('🔇 [GlobalListener] Sound play failed (user interaction required)');
        });
      } catch (e) {
    ...
    // Cleanup on unmount
    return () => {
      console.log('🔌 [GlobalListener] Unsubscribing from realtime channel');
