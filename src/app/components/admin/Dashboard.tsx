...
import { Calendar, DollarSign, Users, Clock, ArrowUpRight, CheckCircle2, MoreHorizontal, Mail, Loader2, Search, X, ChevronLeft, ChevronRight, RefreshCw, Download, Filter, ArrowUpDown } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const handleRefresh = async () => {
    setIsStaffMenuOpen(false);
    toast.loading('Refreshing staff data...', { id: 'staff-refresh' });
    await refetch();
    toast.success('Staff data refreshed!', { id: 'staff-refresh' });
  };
  ...
  const filteredStaffStatus = useMemo(() => {
    ...
    if (staffSearchQuery) {
      filtered = filtered.filter(staff => staff.name.toLowerCase().includes(staffSearchQuery.toLowerCase()));
    }
    if (statusFilter !== 'all') {
                 ...
                 <div className="space-y-4">
                    {currentStaffStatus.length > 0 ? (
                      currentStaffStatus.map((staff, i) => (
                         <div key={i} className="flex items-start justify-between py-2">
                            <div className="flex items-center gap-3">
                               <div className={`w-2.5 h-2.5 rounded-full mt-1 ${staff.color}`} />
                               <div>
                                  <p className="font-semibold text-gray-900">{staff.name}</p>
                                  {staff.busyUntil && (
                                     <p className="text-xs text-gray-400 mt-0.5">Until {staff.busyUntil}</p>
                                  )}
                               </div>
                            </div>
                            <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                               staff.isBusy ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'
                            }`}>
                               {staff.status}
                            </span>
                         </div>
                      ...
                      <div className="text-center py-8">
                         ...
                         <p className="text-sm text-gray-500">No staff found matching "{staffSearchQuery}"</p>
                      </div>
                    ) : (
                      <div className="text-center py-8">
                 ...
                 {totalPages > 1 && (
                    <div className="space-y-2 pt-2 border-t border-gray-100">
                       <div className="text-xs text-gray-500 text-center">
                          Showing {(currentPage - 1) * itemsPerPage + 1}-{Math.min(currentPage * itemsPerPage, filteredStaffStatus.length)} of {filteredStaffStatus.length} technicians
                       </div>
                       <div className="flex items-center justify-between">
                          <Button
                             variant="ghost"
