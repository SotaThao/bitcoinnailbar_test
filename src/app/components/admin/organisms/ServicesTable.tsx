...
export function ServicesTable({
  ...
  const groupedServices = React.useMemo(() => {
    ...
    const categoryAddons = services.filter(
      (s) => s.serviceType === "addon",
    );
    ...
    const standaloneAddons = categoryAddons.filter(
      (addon) => !processedAddonIds.has(addon.id),
    );
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-gray-50 text-gray-500 font-medium uppercase text-xs border-b border-gray-100">
            <tr>
              ...
              <th className="px-6 py-4 text-[#FF9F1C]">
                Member Price
              </th>
