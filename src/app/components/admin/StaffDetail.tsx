...
import { ChevronLeft, Camera, Mail, Phone, Calendar as CalendarIcon, User, Briefcase, DollarSign, Award, Clock, MapPin, Save, X, MoreHorizontal, Star } from 'lucide-react';
...
const ProfileField = ({ icon: Icon, label, value, field, type = "text", required = false, prefix = "", isEditing, onChange, nameInputRef }: ProfileFieldProps) => {
  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors px-4 -mx-4">
      <div className="flex items-center gap-3 w-1/3">
        <div className={`p-1.5 rounded-full ${isEditing ? 'bg-orange-50 text-orange-500' : 'bg-white border border-gray-100 text-gray-400'}`}>
        ...
        {isEditing ? (
           <div className="relative flex justify-end w-full">
             {type === 'date' ? (
               ...
               <>
                 {prefix && <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm z-10">{prefix}</span>}
                 <Input 
                  ...
                  type={type}
                  placeholder={label}
                  maxLength={field.toLowerCase().includes('phone') ? 14 : undefined}
                />
               </>
             )}
           </div>
...
export default function StaffDetail({ staff, isCreating = false, onBack, onSave, onDelete }: StaffDetailProps) {
  const [isEditing, setIsEditing] = useState(isCreating);
  const [formData, setFormData] = useState({
    ...staff,
    commissionRate: staff.commissionRate ? (staff.commissionRate * 100).toString() : '60', // Display as 0-100
    rating: staff.rating || '',
    ...
    <div className="max-w-6xl mx-auto min-h-[80vh] animate-in slide-in-from-right-8 duration-300 font-sans">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={onBack} className="hover:bg-gray-100 rounded-full h-10 w-10">
          ...
          <div className="lg:col-span-4 space-y-6 order-1">
              ...
              <div className="px-6 pb-8 -mt-16 flex flex-col items-center text-center">
                 <div className="relative group mb-5">
                   <Avatar className="w-32 h-32 border-[6px] border-white shadow-sm cursor-pointer bg-white">
                     <AvatarImage src={`https://api.dicebear.com/7.x/initials/svg?seed=${formData.name}`} />
                     <AvatarFallback className="bg-red-600 text-white text-3xl font-medium tracking-wider">
                       {getInitials(formData.name || 'New Staff')}
                     </AvatarFallback>
                   </Avatar>
                   {isEditing && (
                 ...
                 <div className="space-y-4 mb-8 w-full">
                    <div className="space-y-1">
                      {isEditing ? (
                        <div className="space-y-1.5">
                           <Label className="text-xs text-gray-400 font-medium uppercase tracking-wider block text-center">Full Name <span className="text-red-500">*</span></Label>
                           <Input 
                             value={formData.name}
                        ...
                        <h2 className="text-2xl font-serif font-bold text-gray-900">{formData.name || 'New Staff Member'}</h2>
                      )}
                    </div>
                    ...
                    <div className="space-y-1">
                      {isEditing ? (
                        <div className="space-y-1.5">
                           <Label className="text-xs text-gray-400 font-medium uppercase tracking-wider block text-center">Role <span className="text-red-500">*</span></Label>
                           <Input 
                             value={formData.role}
                        ...
                        <p className="text-sm text-gray-500 font-medium tracking-wide uppercase">{formData.role || 'Role'}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-1">
                      {isEditing ? (
                        <div className="space-y-1.5">
                           <Label className="text-xs text-gray-400 font-medium uppercase tracking-wider block text-center">Nickname</Label>
                           <Input 
                        ...
                        formData.nickname && <p className="text-sm text-gray-400 italic">"{formData.nickname}"</p>
                      )}
                    </div>
                 </div>
                 ...
                 <div className="grid grid-cols-3 gap-0 w-full border-t border-gray-100 pt-6">
                    <div className="text-center px-2">
                       ...
                       <div className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold mt-1">Comm.</div>
                    </div>
                    <div className="text-center border-l border-gray-100 px-2">
                       ...
                       <div className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold mt-1">Hourly</div>
                    </div>
                    <div className="text-center border-l border-gray-100 px-2">
                       <div className="text-lg font-bold text-gray-900">
                          {isEditing ? (
                       ...
                       <div className="text-[10px] text-gray-400 uppercase tracking-wider font-semibold mt-1">Type</div>
                    </div>
                 </div>
              </div>
            </Card>
            ...
            <Card className="border-none shadow-[0_2px_20px_rgba(0,0,0,0.04)] rounded-3xl bg-white">
               <CardHeader className="py-3 px-5">
                  <CardTitle className="text-xs font-bold text-gray-400 uppercase tracking-widest">Availability</CardTitle>
               </CardHeader>
            ...
            <Card className="border-none shadow-[0_2px_20px_rgba(0,0,0,0.04)] rounded-3xl bg-white">
               <CardHeader className="py-3 px-5">
                  <CardTitle className="text-xs font-bold text-gray-400 uppercase tracking-widest">Specialties</CardTitle>
               </CardHeader>
               <CardContent className="px-5 pb-5 pt-0">
                  <div className="flex flex-wrap gap-2">
                    {(() => {
                      // 1. Define Default Skills
                      const defaultSkills = ['Manicure', 'Pedicure', 'Gel Polish', 'Acrylic', 'Dip Powder', 'Nail Art', 'Nail Extension', 'Spa Treatment'];
                      
                      // 2. Get current staff skills (clean up "+" suffix) to ensure we show everything
                      const staffSkills = (formData.specialties || []).map((s: string) => s.replace('+', ''));
                      
                      // 3. Merge and Unique
                      const allSkills = Array.from(new Set([...defaultSkills, ...staffSkills]));

                      return (
                        <>
                          {allSkills.map((specialty) => {
                            const isLevel1 = formData.specialties.includes(specialty);
                            const isLevel2 = formData.specialties.includes(`${specialty}+`);
                            ...
                            const handleToggle = () => {
                              ...
                              setFormData((prev: any) => {
                                const currentList = prev.specialties || [];
                                let newList = [...currentList];
                                
                                if (isLevel1) {
                                    // Good -> Great
                                    newList = newList.filter((s: string) => s !== specialty);
                            ...
                            return (
                              ...
                              >
                                {specialty}
                                {isLevel1 && <span className="ml-1.5 text-[10px] opacity-70 font-normal">• Good</span>}
                                {isLevel2 && <span className="ml-1.5 text-[10px] opacity-90 font-bold">• Great</span>}
                              </Badge>
                            );
                          })}
                          ...
                          {isEditing && (
                            <Badge
                              ...
                              onClick={() => {
                                const newSkill = prompt("Enter new skill name (e.g., Waxing):");
                                if (newSkill && newSkill.trim()) {
                                  const formattedSkill = newSkill.trim();
                                  // Add immediately as "Good" level
                                  setFormData((prev: any) => ({
                                    ...prev,
                                    specialties: [...(prev.specialties || []), formattedSkill]
                                  }));
                                }
                              }}
                            >
          ...
          <div className="lg:col-span-8 space-y-6 order-2">
            ...
            <Card className="border-none shadow-[0_2px_20px_rgba(0,0,0,0.04)] overflow-hidden rounded-3xl bg-white">
              <CardHeader className="border-b border-gray-50 p-[16px] pt-[16px] pr-[16px] pb-[24px] pl-[16px]">
                <CardTitle className="text-lg font-bold text-gray-900">Contact Details</CardTitle>
              </CardHeader>
              <CardContent className="pt-[0px] pr-[16px] pb-[24px] pl-[16px]">
                <div className="space-y-0">
                  <ProfileField icon={Phone} label="Phone Number" value={formData.phone} field="phone" required isEditing={isEditing} onChange={(val) => setFormData((prev: any) => ({...prev, phone: val}))} />
                  <ProfileField icon={Mail} label="Email Address" value={formData.email} field="email" required isEditing={isEditing} onChange={(val) => setFormData((prev: any) => ({...prev, email: val}))} />
                  <ProfileField icon={User} label="Emergency Contact" value={formData.emergencyContactName} field="emergencyContactName" prefix="Name: " isEditing={isEditing} onChange={(val) => setFormData((prev: any) => ({...prev, emergencyContactName: val}))} />
                  <ProfileField icon={Phone} label="Emergency Phone" value={formData.emergencyContactPhone} field="emergencyContactPhone" prefix="Phone: " isEditing={isEditing} onChange={(val) => setFormData((prev: any) => ({...prev, emergencyContactPhone: val}))} />
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="border-none shadow-[0_2px_20px_rgba(0,0,0,0.04)] overflow-hidden rounded-3xl bg-white">
              <CardHeader className="border-b border-gray-50 p-[16px]">
                <CardTitle className="text-lg font-bold text-gray-900">Professional Details</CardTitle>
              </CardHeader>
              <CardContent className="pt-[0px] pr-[16px] pb-[24px] pl-[16px]">
                <div className="space-y-0">
                   ...
                   <ProfileField icon={Award} label="License Number" value={formData.licenseNumber} field="licenseNumber" required isEditing={isEditing} onChange={(val) => setFormData((prev: any) => ({...prev, licenseNumber: val}))} />
                   <ProfileField icon={Star} label="Rating (0-5)" value={formData.rating} field="rating" type="number" isEditing={isEditing} onChange={(val) => setFormData((prev: any) => ({...prev, rating: val}))} />
                   <ProfileField icon={CalendarIcon} label="Date Hired" value={formData.hireDate} field="hireDate" type="date" required isEditing={isEditing} onChange={(val) => setFormData((prev: any) => ({...prev, hireDate: val}))} />
                   <ProfileField icon={DollarSign} label="Tip Split (%)" value={formData.tipSplit} field="tipSplit" isEditing={isEditing} onChange={(val) => setFormData((prev: any) => ({...prev, tipSplit: val}))} />
                </div>
              </CardContent>
            </Card>
