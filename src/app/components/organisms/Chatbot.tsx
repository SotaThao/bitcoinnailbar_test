...
export function Chatbot() {
  ...
  useEffect(() => {
    // Only reset if messages are empty OR if the only message is the welcome message
    // But for simplicity in this demo, let's just clear and add the new welcome message to ensure it matches the language
    setMessages([
      { 
        role: 'assistant', 
  ...
  return (
    <>
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed right-4 bottom-36 md:bottom-24 z-50 w-[350px] md:w-[400px] h-[500px] bg-[#1a1f2e] border border-[#FF9800]/30 rounded-2xl shadow-2xl flex flex-col overflow-hidden font-sans"
      ...
      <AnimatePresence>
        {showBubble && !isOpen && (
          <motion.div
            ...
            className="fixed right-6 bottom-42 md:bottom-24 z-50 bg-white text-black px-4 py-3 rounded-xl shadow-[0_0_20px_rgba(255,152,0,0.3)] border-2 border-[#FF9800] max-w-[200px] pointer-events-none"
          >
      ...
      <motion.button
        ...
        className="fixed right-6 bottom-24 md:bottom-6 z-50 w-16 h-16 rounded-full bg-[#FF9800] shadow-lg flex items-center justify-center overflow-hidden border-4 border-[#FF9800]/50"
