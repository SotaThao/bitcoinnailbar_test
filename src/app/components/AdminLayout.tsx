        ...
        <div className="flex h-20 items-center gap-3 px-6">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="font-serif font-bold text-xl tracking-tight text-gray-900 uppercase">BITCOIN</span>
              <span className="text-[10px] font-bold text-gray-500 tracking-[0.3em] uppercase">NAIL BAR</span>
            </div>
          </div>
        </div>
                ...
                >
                  ...
                  <div className="flex flex-1 items-center justify-between w-full">
                    ...
                    {link.label === 'Analytics' && (
                      <span className="text-[10px] italic text-gray-400 font-normal ml-2">Coming soon</span>
                    )}
                  </div>
                </Button>
              </Link>
      ...
      <div className="flex-1 md:pl-64 flex flex-col min-h-screen">
        ...
        <header className="hidden md:flex h-20 items-center justify-between px-8 bg-white border-b border-gray-100 sticky top-0 z-40">
           <div className="flex items-center gap-3">
              <div className="p-2 bg-gray-50 rounded-lg">
                 ...
                 {navLinks.find(l => l.path === location.pathname)?.icon ? (
                    (() => {
                       const Icon = navLinks.find(l => l.path === location.pathname)!.icon;
              ...
              <h1 className="text-xl font-serif font-bold text-gray-900">{currentPage}</h1>
           </div>
              ...
              <div className="flex items-center gap-3 pl-6 border-l border-gray-100">
                 ...
                 <Avatar className="h-10 w-10 border-2 border-white shadow-sm cursor-pointer">
                    ...
                    <AvatarFallback className="bg-[#FFF7ED] text-[#F97316]">
                      {currentUser?.full_name?.split(' ').map(n => n[0]).join('').toUpperCase() || 'AD'}
                    </AvatarFallback>
                 </Avatar>
              </div>
           </div>
        ...
        <header className="sticky top-0 z-40 md:hidden flex h-16 items-center justify-between border-b border-gray-200 bg-white px-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-serif font-bold text-gray-900 tracking-wider">ADMIN</span>
          </div>
          <div className="flex items-center gap-2">
                ...
                <Link key={link.path} to={link.path}>
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1">
                      <span>{link.label}</span>
                      {link.label === 'Analytics' && (
                        <span className="text-[10px] italic font-normal opacity-70">Coming soon</span>
                      )}
                    </div>
                  </Button>
                </Link>
