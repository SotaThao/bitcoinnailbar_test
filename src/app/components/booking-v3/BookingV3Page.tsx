/**
 * Booking v3 — menu dịch vụ + tuỳ chỉnh add-on + gợi ý nâng gói theo delta price.
 *
 * Port từ `src/components/public/booking/PosMenuUpsellPreviewPage.tsx` của
 * NEXORA TOUCH (vlink-nexora-fe, branch `feature/800_pos-menu-upsell`,
 * commit d446df17). Bản đó chạy ở route nội bộ /preview/menu và dùng bảng màu
 * vàng-nâu của Bitcoin Nail Bar — đúng chỗ của repo này, nên đưa về đây.
 *
 * Khác bản gốc đúng 3 điểm, không đổi gì về giao diện hay logic tính giá:
 * - Nguồn dữ liệu là menu demo tĩnh (`lib/booking-v3/demo-menu`) thay cho
 *   TanStack Query hooks của POS. Nối API service-menu thật là việc tiếp theo.
 *   Bản gốc cũng chỉ hiển thị demo vì hook luôn rỗng ở route public.
 * - Router `react-router` (repo này) thay cho `react-router-dom`.
 * - Nút quay lại trỏ `/admin/services` thay cho `/dashboard/pos/services`.
 * - Ảnh category đổi từ `/prototype/assets/cat-*.jpg` (đường dẫn của repo kia)
 *   sang `/cat-*.jpg` — đúng chỗ 5 ảnh đã có sẵn trong `public/` ở đây, nếu
 *   không thì mọi thumbnail rơi vào 404.
 *
 * Bản gốc dựng `displayOrder` cho dữ liệu demo bằng tên field sai
 * (`businessId`/`sortOrder`) nên `tsc` báo 2 lỗi TS2322/TS2352; ở đây dùng
 * đúng tên field của type nên hết lỗi.
 */
...
import { DEMO_CATEGORIES, DEMO_SERVICES } from '@/app/lib/booking-v3/demo-menu'
import type { PosCategory, PosService } from '@/app/lib/booking-v3/types'
import {
  extractMemberPriceFromService,
  parseLinkedAddons,
} from '@/app/lib/booking-v3/menu-helpers'
...
export default function BookingV3Page() {
  ...
  const getCategoryPhoto = (catName: string) => {
    ...
    if (lower.includes('pedi')) return '/cat-pedicure.jpg'
    if (lower.includes('mani')) return '/cat-manicure.jpg'
    if (
      ...
      return '/cat-enhancements.jpg'
    if (lower.includes('wax')) return '/cat-waxing.jpg'
    if (lower.includes('kid')) return '/cat-kids.jpg'
    return '/cat-pedicure.jpg'
  }
  ...
  const getServicePhoto = (service: PosService | null | undefined): string => {
    if (!service) return '/cat-pedicure.jpg'
