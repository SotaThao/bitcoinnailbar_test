...
export function PaymentModal({
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === "payment-success") {
        ...
        console.log(
          "📥 [MODAL] Received close-modal message from iframe",
        );
        onClose();
      }
    };
  ...
  const handleEmailSubmit = () => {
    if (!email.trim()) {
      setEmailError(
        t("payment_modal.email_form.error_required"),
      );
      return;
    }
    ...
    if (!validateEmail(email)) {
      setEmailError(
        t("payment_modal.email_form.error_invalid"),
      );
      return;
    }

    // Replace {email} placeholder in payment URL
    const urlWithEmail = paymentUrl.replace(
      "{email}",
  ...
  const handlePaymentSuccess = (redeemCode: string) => {
    ...
    setTimeout(() => {
      const redeemSection =
        document.getElementById("redeem-section");
      if (redeemSection) {
  ...
  const handleCopyUrl = async () => {
    try {
      ...
      console.log(
        "✅ [COPY] Payment URL copied to clipboard:",
        iframeUrl,
      );
      setCopied(true);
      ...
      try {
        ...
        console.log(
          "✅ [COPY] Payment URL copied to clipboard (fallback):",
          iframeUrl,
        );
        setCopied(true);
        setTimeout(() => setCopied(false), 2000);
  ...
  const handleOpenInNewTab = () => {
    console.log(
      "🔗 [OPEN TAB] Opening payment URL in new tab:",
      iframeUrl,
    );
        ...
        <div className="flex items-center justify-between p-4 md:p-6 border-b border-gray-200 bg-gradient-to-r from-[#FF9800] to-[#F57C00]">
          <div>
            <h2 className="text-base font-bold text-white">
              ...
              {t("payment_modal.header.membership")}
            </h2>
            <p className="text-xs text-white/90 mt-0.5">
              {t("payment_modal.header.payment")} $
            ...
            <div className="max-w-md mx-auto space-y-6 py-6 md:py-8">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-700">
                  {t("payment_modal.email_form.label")}{" "}
                  <span className="text-red-500">
                    {t("payment_modal.email_form.required")}
                  </span>
                </label>
                <Input
                  type="email"
                  ...
                  placeholder={t(
                    "payment_modal.email_form.placeholder",
                  )}
            ...
            <div className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 md:p-4">
                <p className="text-sm text-blue-800">
                  📧 {t("payment_modal.payment_iframe.sent_to")}{" "}
                  <strong>{email}</strong>
                </p>
              </div>
