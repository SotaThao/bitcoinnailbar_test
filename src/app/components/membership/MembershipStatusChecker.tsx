...
export function MembershipStatusChecker() {
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      const queryValue = userId.includes("@")
        ? userId.trim()
        : rawValue;
      ...
      console.log(
        "🔍 [MEMBERSHIP CHECK] Full API Response:",
        JSON.stringify(result, null, 2),
      );
      console.log(
        "🔍 [MEMBERSHIP CHECK] HTTP Status:",
        response.status,
      );
      console.log(
        "🔍 [MEMBERSHIP CHECK] response.ok:",
        response.ok,
      );
      console.log(
        "🔍 [MEMBERSHIP CHECK] result.success:",
        result.success,
      );
      console.log(
        "🔍 [MEMBERSHIP CHECK] has_membership:",
        result.has_membership,
      );
      console.log("🔍 [MEMBERSHIP CHECK] data:", result.data);
      console.log(
        "🔍 [MEMBERSHIP CHECK] data.membership:",
      ...
      if (!result.success) {
        throw new Error(
          result.error || "Failed to fetch membership",
        );
      }
      ...
      if (result.has_membership && result.data?.membership) {
        console.log(
          "✅ [MEMBERSHIP CHECK] Membership found, setting state",
        );
        setMembership({
        ...
        console.log(
          "⚠️ [MEMBERSHIP CHECK] No membership found in response",
        );
        setMembership({
        ...
        <div className="p-6 bg-gradient-to-br from-[#1f2937] to-[#111827] rounded-2xl border border-gray-700 space-y-4">
          {membership.activeMembership ? (
            <>
              {/* Active Badge */}
              <div
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r ${getTierColor(membership.activeMembership.tier)} text-white font-bold text-sm`}
              >
                <Crown className="w-4 h-4" />
                {membership.activeMembership.tier.toUpperCase()}{" "}
                MEMBER
              </div>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex items-center gap-2 text-gray-300">
                  ...
                  <span className="font-semibold text-white">
                    {new Date(
                      membership.activeMembership.expires_at,
                ...
                {membership.totalActive > 1 && (
                  <div className="p-3 bg-blue-900/20 border border-blue-700/30 rounded-lg">
                    <p className="text-xs text-blue-300">
                      ℹ️ Bạn có {membership.totalActive} gói
                      membership. Gói thấp hơn sẽ tự động kích
                      hoạt khi gói hiện tại hết hạn.
                    </p>
                  </div>
                )}
              </div>
              ...
              {membership.allMemberships &&
                membership.allMemberships.length > 1 && (
                    ...
                    <div className="space-y-2">
                      {membership.allMemberships
                        ...
                        .map((m: any, index: number) => (
                          ...
                          >
                            <span
                              className={`font-medium ${m.status === "active" ? "text-[#FF9800]" : "text-gray-400"}`}
                            >
                              {m.tier.toUpperCase()}{" "}
                              {m.status === "active"
                                ? "(Active)"
