      ...
      try {
        const [coingeckoRes, vlinkRes] =
          await Promise.allSettled([
            fetch(
              "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin,ethereum,ripple,binancecoin,solana,cardano,binance-peg-dogecoin",
            ),
            fetch(
              `https://${projectId}.supabase.co/functions/v1/make-server-84f9c112/proxy/vlink`,
              {
                headers: {
            ...
            const findAllMarkets = (data: any): any[] => {
              ...
              const traverse = (node: any) => {
                ...
                const hasPrice =
                  ...
                  node.last_price !== undefined ||
                  node.changedPercent24 !== undefined;
            ...
            if (markets.length > 0) {
              ...
              const vmmMarket = markets.find((m: any) => {
                const baseName = normalize(
                  m.baseName || m.base_name,
                );
                const symbol = normalize(m.symbol || m.pair);
                // Match VMM
                return (
                  symbol.includes("VMM") ||
                  baseName.includes("VMM")
                );
              });
              ...
              const vlgMarket = markets.find((m: any) => {
                ...
                return (
                  symbol.includes("VLG") ||
                  baseName.includes("VLG")
                );
              });

              const processVLinkItem = (
                market: any,
                displaySymbol: string,
                color: string,
                Icon: any,
              ): CryptoItemProps | null => {
              ...
              const vmmItem = processVLinkItem(
                vmmMarket,
                "VMM/USDT",
                "#8b5cf6",
                Wallet,
              ); // Purple
              const vlgItem = processVLinkItem(
                vlgMarket,
                "VLG/USDT",
                "#ec4899",
                Zap,
        ...
        const hasVMM = vlinkData.some(
          (d) => d.symbol === "VMM/USDT",
        );
        const hasVLG = vlinkData.some(
          (d) => d.symbol === "VLG/USDT",
        );

        if (!hasVMM) {
          console.warn(
            "⚠️ VMM not found in API data, using fallback",
          );
          vlinkData.unshift({
            icon: <Wallet size={12} />,
            symbol: "VMM/USDT",
        ...
        if (!hasVLG) {
          console.warn(
            "⚠️ VLG not found in API data, using fallback",
          );
          // Add after VMM
          const insertIdx =
            vlinkData.findIndex(
