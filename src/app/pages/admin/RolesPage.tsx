...
import { Plus, Pencil, Trash2, Shield, X, CheckSquare, Square } from 'lucide-react';
...
export default function RolesPage() {
  ...
  const fetchRoles = async () => {
    try {
      ...
      console.log('🔍 [ROLES] Fetching roles with token:', token ? 'YES' : 'NO');
      ...
      console.log('🔍 [ROLES] Roles response:', data);
      
      if (data.success) {
        console.log('✅ [ROLES] Roles loaded:', data.data.length);
        setRoles(data.data);
      } else {
        console.error('❌ [ROLES] Failed to fetch roles:', data.error);
        setError(data.error || 'Failed to fetch roles');
      }
    } catch (err: any) {
      console.error('❌ [ROLES] Exception fetching roles:', err);
  ...
  const fetchPermissions = async () => {
    try {
      const token = getAuthToken();
      console.log('🔍 [ROLES] Fetching permissions with token:', token ? 'YES' : 'NO');
      ...
      const data = await response.json();
      console.log('🔍 [ROLES] Permissions response:', data);
      
      if (data.success) {
        console.log('✅ [ROLES] Permissions loaded:', data.data.length);
        setPermissions(data.data);
      } else {
        console.error('❌ [ROLES] Failed to fetch permissions:', data.error);
      }
    } catch (err: any) {
      console.error('❌ [ROLES] Exception fetching permissions:', err);
    }
  };
  ...
  return (
    <AdminLayout>
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">Create and manage user roles with custom permissions</p>
            </div>
            <Button
              onClick={() => {
                setFormData({ name: '', description: '', permissions: [] });
        ...
        {/* Create Role Modal */}
        {showCreateModal && (
          <div className="fixed inset-0 bg-[rgba(0,0,0,0.2)] bg-opacity-50 flex items-center justify-center z-50 p-4">
