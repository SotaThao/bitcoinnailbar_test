...
utilitiesApp.post("/make-server-84f9c112/chat", async (c) => {
  try {
    ...
    const flattenServices = (data: any): any[] => {
      ...
      Object.keys(data).forEach((categoryKey) => {
        // FILTER: Skip inactive categories
            ...
            if (group.items) {
              group.items.forEach((item: any) => {
                ...
                const parsePrice = (val: any): number => {
                  ...
                  if (str.includes('-')) {
                    const parts = str.split('-').map((s: string) => parseFloat(s.replace(/[^\d.]/g, '')));
                    return Math.min(...parts.filter((n: number) => !isNaN(n)));
                  }
                ...
                flattened.push({
                  ...
                  name_en: item.name_en || item.name || 'Unknown',
                  name_vi: item.name_vi || item.name || 'Không rõ',
    ...
    const services = flattenServices(serviceMenuData);

    // Sort by price (ascending)
    services.sort((a, b) => a.price - b.price);

    console.log(`🤖 [CHAT] Loaded ${services.length} services for chatbot context`);

    // Build system prompt
    const systemPrompt = `You are a helpful assistant for Bitcoin Nail Bar, a premium nail salon.

SERVICES (${services.length} total):
${services.map(s => `- ${s.name_en} (${s.name_vi}) | $${s.price} | ${s.duration} min | Category: ${s.category}`).join('\n')}

RULES:
1. Answer ONLY about services listed above
2. Be concise and friendly
3. If asked about pricing, use exact prices from the list
4. If service not found, politely say "We don't offer that service"
5. Use ${language === 'vi' ? 'Vietnamese' : 'English'}
6. DO NOT make up prices or services`;
    ...
    if (!response.ok) {
      ...
      console.error("❌ [CHAT] DeepSeek API error:", errorText);
      return c.json({ 
        success: false, 
    ...
    console.log(`✅ [CHAT] Response generated (${language})`);
